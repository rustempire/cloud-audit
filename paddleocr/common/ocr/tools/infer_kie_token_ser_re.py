...
from common.ocr.ppocr import build_model
from common.ocr.ppocr.postprocess import build_post_process
from common.ocr.ppocr import load_model
from common.ocr.ppocr import draw_re_results
from common.ocr.ppocr. utils.logging import get_logger
from common.ocr.ppocr import get_image_file_list, print_dict
from common.ocr.tools.program import ArgsParser, load_config, merge_config
from common.ocr.tools.infer_kie_token_ser import SerPredictor
...
def make_input(ser_inputs, ser_results):
    ...
    ser_results = ser_results["Teacher"][0]
...
class SerRePredictor(object):
    ...
    def __call__(self, data):
        ...
        # if self.model.use_visual_backbone is False:
        #     re_input.pop(4)
        re_input.pop(4)
        preds = self.model(re_input)
        post_result = self.post_process_class(preds, ser_results=ser_results["Teacher"], entity_idx_dict_batch=entity_idx_dict_batch)
        return post_result
...
if __name__ == '__main__':
    ...
    with open(
            os.path.join(config['Global']['save_res_path'], "infer_results.txt"), "w", encoding='utf-8') as fout:
